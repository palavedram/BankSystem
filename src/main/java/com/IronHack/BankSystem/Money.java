...
public class Money {
    ...
    public Money multiply(int i){
        BigDecimal result = this.amount.multiply(new BigDecimal(i));
        return new Money(result,this.currency,DEFAULT_ROUNDING);
    }

    public Money multiply(double d){
        BigDecimal result = this.amount.multiply(new BigDecimal(d));
        return new Money(result,this.currency,DEFAULT_ROUNDING);
    }

    public Money divide(int i) {
        BigDecimal result = this.amount.divide(new BigDecimal(i), DEFAULT_ROUNDING);
        return new Money(result, this.currency, DEFAULT_ROUNDING);
    }
}